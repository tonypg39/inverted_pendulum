...
def force():
    ...
    if mode == 'manual':
        ...
        response['F'] = pid_theta.getSignalControl(float(set_point['theta']), float(
            pend_state['theta']), float(sim_params['dt']), float(sim_params['max_force']))
    elif mode == 'pid_cascade':
        temp = pid_x_dot.getSignalControl(float(set_point['x_dot']), float(
            pend_state['x_dot']), float(sim_params['dt']), float(sim_params['max_force']))
        response['F'] = pid_theta.getSignalControl(temp, float(
            pend_state['theta']), float(sim_params['dt']), float(sim_params['max_force']))
    else:
        response['F'] = 0.0
